...
contract CharacterNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    struct CharacterType {
        ...
        string metadataURI; // IPFS URI for this character
        bool exists;
    }
    ...
    constructor(
        ...
        // Initialize default characters (metadataURI set later via setCharacterMetadataURI)
    ...
    function mintWithCoins(uint8 characterType) external {
        ...
        // Burn coins directly from user's wallet (requires prior approve)
        gameCoin.burnFrom(msg.sender, price);
    ...
    function addCharacterType(
        ...
        _addCharacterType(newId, name, rarity, price, metadataURI);
    }

    /**
     * @notice Set metadata URI for a character type
     */
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // Return character-specific metadata URI
        if (bytes(charType.metadataURI).length > 0) {
            return charType.metadataURI;
        }
        
        // Fallback to baseURI pattern
